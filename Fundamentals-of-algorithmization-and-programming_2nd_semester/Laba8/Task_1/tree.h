...
class AVLTree {
   public:
    ...
    QString traverse(TraversalType type = IN_ORDER) const {
        ...
        switch (type) {
            case IN_ORDER:
                traverseInOrder(root_.get(), result);
                break;
            case PRE_ORDER:
                traversePreOrder(root_.get(), result);
                break;
            case POST_ORDER:
                traversePostOrder(root_.get(), result);
                break;
            case LEVEL_ORDER:
    ...
    void insertSubtree(const Key& parentKey, bool isLeft,
                       const AVLTree& subtree) {
        Node* parent = findNode(root_.get(), parentKey);
        if (!parent)
            return;
        if (isLeft)
            parent->left.reset(copyTree(subtree.root_.get(), parent));
        else
            parent->right.reset(copyTree(subtree.root_.get(), parent));
    }
    ...
    void removeSubtree(const Key& rootKey) {
        ...
        if (target == root_.get())
            root_.reset();
        else if (target->parent->left.get() == target)
    ...
    void insertBranch(const Key& parentKey, bool isLeft,
                      const AVLTree& branch) {
        Node* parent = findNode(root_.get(), parentKey);
        if (!parent || !branch.root_)
            return;
        Node* cloned = copyTree(branch.root_.get(), parent);
        if (isLeft)
            parent->left.reset(cloned);
        else
            parent->right.reset(cloned);
    }
    ...
    void insertAtPosition(const Key& parentKey, bool isLeft, Key newKey,
                          Value newVal) {
        Node* parent = findNode(root_.get(), parentKey);
        if (!parent)
            return;
        auto newNode = std::make_unique<Node>(std::move(newKey),
                                              std::move(newVal), parent);
   ...
   private:
    ...
    int balanceFactor(const Node* n) const noexcept {
        return height(n->right.get()) - height(n->left.get());
    }
    ...
    Node* findMin(Node* n) const noexcept {
        while (n && n->left)
            n = n->left.get();
        return n;
    }
    ...
    void traverseInOrder(Node* n, QString& res) const {
        if (!n)
            return;
        traverseInOrder(n->left.get(), res);
        if (!res.isEmpty())
            res += ' ';
